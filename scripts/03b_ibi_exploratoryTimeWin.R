...
dataIBI <- data.frame(
  ...
  Av_allTr = rowMeans(subset(importIBI, select = ibi_Akq_51_9:ibi_Akq_51_14)),
  Av_1stBl = rowMeans(subset(importIBI, select = ibi_Akq1_51_9:ibi_Akq1_51_14)),
  Av_2ndBl = rowMeans(subset(importIBI, select = ibi_Akq2_51_9:ibi_Akq2_51_14)),
  Neu_allTr = rowMeans(subset(importIBI, select = ibi_Akq_52_9:ibi_Akq_52_14)),
  Neu_1stBl = rowMeans(subset(importIBI, select = ibi_Akq1_52_9:ibi_Akq1_52_14)),
  Neu_2ndBl = rowMeans(subset(importIBI, select = ibi_Akq2_52_9:ibi_Akq2_52_14)),
  Min_allTr = rowMeans(subset(importIBI, select = ibi_Akq_53_9:ibi_Akq_53_14)),
  Min_1stBl = rowMeans(subset(importIBI, select = ibi_Akq1_53_9:ibi_Akq1_53_14)),
  Min_2ndBl = rowMeans(subset(importIBI, select = ibi_Akq2_53_9:ibi_Akq2_53_14))
)  
...
save_as_docx(tableIBI, path = paste0(pathname, "/tables/tableIBI_exploratoryTimeWin_raw.docx"))
...
graphIBIima <- ggplot(data = ibiGAacqIma, aes(x = time, y = IBI, colour = csType)) + 
  theme_classic() +
  geom_rect(xmin = 4, xmax = 7, ymin = yMin, ymax = yMax, fill = "gray90", colour = NA) +
...
graphIBImeansIma <- ggplot(data = meanIBI[meanIBI$usGroup == "Imagery-Based",], aes(x = CS, y = mean, fill = CS)) +
  ...
  scale_y_continuous(name = "Mean IBI change (4-7 s)", limits = c(yMinBar, yMaxBar)) +
...
if (showSig == TRUE){
  graphIBImeansIma <- graphIBImeansIma +
    geom_segment(aes(x = 1, y = mean+se+1.5, xend = 3, yend = mean+se+1.5), data = meanIBI[1,]) +
    geom_text(aes(label = "†", x = 2, y = mean+se+4.5), size = plotFS/3, data = meanIBI[1,], family = "Helvetica")
  graphIBImeansReal <- graphIBImeansReal +  
    geom_segment(aes(x = 1, y = mean+se+1.5, xend = 2, yend = mean+se+1.5), data = meanIBI[4,]) +
    geom_text(aes(label = "***", x = 1.5, y = mean+se+1.6), size = plotFS/2, data = meanIBI[4,]) +
    geom_segment(aes(x = 1, y = mean+se+4.5, xend = 3, yend = mean+se+4.5), data = meanIBI[4,]) +
    geom_text(aes(label = "***", x = 2, y = mean+se+4.6), size = plotFS/2, data = meanIBI[4,])
}
...
# saving it
ggsave(filename = paste0(pathname, "/figures/Figure4b_timeCourses_barPlot_IBI_exploratory.eps"),
       plot = graphIBI,
       width = 150,
       height = 150,
       units = "mm"
...
ggsave(filename = paste0(pathname, "/figures/Figure4b_timeCourses_barPlot_IBI_exploratory.pdf"),
       plot = graphIBI,
       width = 150,
       height = 150,
       units = "mm"
