...
# --- content: Main analyses on IBI with exploratory time window (ANOVAs, pairwise comparisons, plotting)
...
graphIBIdiffIma <- ggplot(data = dataIBIdiff[dataIBIdiff$usGroup == "ima",], aes(x = comp, y = IBI)) +
  ...
  scale_y_continuous(name = "Difference in mean IBI change (4-7 s)", limits = c(yMinDiff, yMaxDiff+10)) +
...
if (showSig == TRUE){
  ...
  graphIBIdiffReal <- graphIBIdiffReal +  
    geom_text(aes(label = "***", x = 1, y = yMaxDiff+5), size = plotFS/1.5, color = "darkred") +
